...
internal class TransactionDao(context: Context) : SQLiteOpenHelper(context, "financial_tracker.db", null, 7) {
    ...
    suspend fun checkDuplicate(refId: String): FinancialTransaction? =
        withContext(Dispatchers.IO) {
            ...
            if (cursor.moveToFirst()) {
                ...
                t =
                    FinancialTransaction(
                        ...
                        balance =
                            cursor.getColumnIndex("balance").takeIf { it >= 0 }?.let {
    ...
    suspend fun getAll(): List<FinancialTransaction> =
        withContext(Dispatchers.IO) {
    ...
    suspend fun insertUnparsedNotification(notification: UnparsedNotification) =
        withContext(Dispatchers.IO) {
            val db = writableDatabase
            val values =
                ContentValues().apply {
