...
class _SmartDashboardScreenState extends State<SmartDashboardScreen> {
          ...
          : StreamBuilder<Map<String, dynamic>>(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasData && snapshot.data != null) {
                  ...
                  // Avoid duplicates by checking ID or timestamp if available
                  // For now, we prepend it to the list.
                  final existingIndex = _transactions.indexWhere((t) => t['timestamp'] == newTx['timestamp']);
                  if (existingIndex == -1) {
                    _transactions.insert(0, newTx);
                    HapticFeedback.heavyImpact(); // Aha Moment: Haptic feedback when a transaction arrives!
                  }
                }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Padding(
                          ...
                          child: Text(
                            'أي عملية دفع تقوم بها عبر (STC Pay أو جيب) ستظهر هنا تلقائياً دون تدخل منك.',
  ...
  List<Widget> _buildDynamicCategoryChips(Map<String, dynamic> tx, {required bool isOutbound}) {
    if (!isOutbound) {
      // For inbound (إيراد)
      return [
        _buildQuickCategoryChip(CategoryModel(name: 'تسديد دين لي', type: 'general', iconCodePoint: Icons.account_balance_wallet.codePoint), tx, isDebt: true),
        // _buildQuickCategoryChip(CategoryModel(name: 'إيراد', type: 'income', iconCodePoint: Icons.add_circle.codePoint), tx, isGeneralExpense: true),
      ];
    }
    ...
    return [
      if (!isUtility)
        _buildQuickCategoryChip(CategoryModel(name: 'سداد دين علي', type: 'general', iconCodePoint: Icons.money_off.codePoint), tx, isDebt: true),
      _buildQuickCategoryChip(CategoryModel(name: 'تسجيل كمصروف', type: 'expense', iconCodePoint: Icons.receipt_long.codePoint), tx, isGeneralExpense: true),
    ];
  }

  Widget _buildQuickCategoryChip(CategoryModel category, Map<String, dynamic> tx, {bool isDebt = false, bool isGeneralExpense = false}) {
    final bool isPrimary = isDebt;
